<template>
	<view class="page">
		...
		<image src="../../static/logo-2.png" style="width: 50rpx;height: 50rpx;" @click="close()"></image>
		...
		methods: {
			...
			getCode() {
				...
				if (mobile == null || mobile.length == 0) {
					...
					return;
				} else if (/^[0-9]{11}$/.test(mobile) == false) {
					console.log("hello");
					uni.showToast({
						icon: 'none',
						title: '手机号码必须是6位数字'
				...
				var serverUrl = me.url.baseUrl;
				// 调用后端发送验证码
				console.log(serverUrl+ "/passport/getSMSCode?mobile=" + mobile)
				uni.request({
					method: "POST",
			...
			loginOrRegist() {
				...
				// 				// me.ajax(serverUrl + "/passport/login", 'POST', data, function(resp) {
